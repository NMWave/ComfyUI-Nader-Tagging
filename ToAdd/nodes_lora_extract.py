...
def extract_lora(diff, rank):
    ...
    U, S, V = torch.svd_lowrank(diff_float, q=rank, niter=7)
...
class LoraSave:
    ...
    def save(self, filename_prefix, rank, lora_type, bias_diff, model_diff=None, text_encoder_diff=None):
        if model_diff is None and text_encoder_diff is None:
            logging.info("LoraSave: No model or text encoder diff provided. Nothing to do.")
            return {}

        lora_type_enum = LORA_TYPES.get(lora_type)
        if lora_type_enum == LORAType.STANDARD:
            logging.info(f"Starting LoRA extraction with rank={rank}. This may take a long time...")
        else:
        ...
        if model_diff is not None:
            ...
            output_sd = calc_lora_model(model_diff, rank, "diffusion_model.", "diffusion_model.", output_sd, lora_type_enum, bias_diff=bias_diff)
            
        if text_encoder_diff is not None:
            logging.info("\n--- Processing Text Encoders (CLIP) ---")
            output_sd = calc_lora_model(text_encoder_diff.patcher, rank, "text_model.", "text_model.", output_sd, lora_type_enum, bias_diff=bias_diff)
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "LoraSave": "Extract and Save Lora (with progress)"
}